...
.. note:: For HA HDFS deployments you will need to include ``hadoop.conf.dir``, setting it to a directory which includes both hdfs-site.xml. Once hdfs-site.xml is in place and ``hadoop.conf.dir`` has been set ``hdfs.url`` may be set to the namenodes nameservice id. i.e. 'nameservice1' . 
