...
public class HdfsSinkConnectorConfig extends AbstractConfig {
  ...
  private static final String PARTITION_TIME_FIELD_NAME_DOC =
      "The name of the field in the sink record that contains the timestamp as "
      + "milliseconds past epoch to be used for partitioning when TimeBasedPartition used. "
      + "When this is empty, the partitioning will use the current time the record is processed.";
      ...
      try {
        ...
        if (classNameEquals(partitionerName, DefaultPartitioner.class)) {
          ...
          if (classNameEquals(partitionerName, DailyPartitioner.class) || classNameEquals(partitionerName, HourlyPartitioner.class)) {
            ...
            return name.equals(PARTITION_TIME_FIELD_NAME_CONFIG) || name.equals(PARTITION_DURATION_MS_CONFIG)
                    || name.equals(PATH_FORMAT_CONFIG) || name.equals(LOCALE_CONFIG) || name.equals(TIMEZONE_CONFIG);
          }
        } else {
