...
public class HdfsSinkConnectorConfig extends AbstractConfig {
  ...
  private static final String KERBEROS_TICKET_RENEW_PERIOD_MS_DISPLAY =
      "Kerberos Ticket Renew Period (ms)";
  ...
  private static final String ROTATE_SCHEDULE_INTERVAL_MS_DOC =
      ...
      + "ensures that "
      + "file commits are invoked every configured interval. Time of commit will be adjusted to "
      + "00:00 of selected timezone. "
      + "Commit will be performed at scheduled time regardless previous commit time or number of "
      + "messages. "
      + "This configuration is useful when you have to commit your data based on current server "
      + "time, like at the beginning of every hour. "
      + "The default value -1 means that this feature is disabled.";
  ...
  private static final String PATH_FORMAT_DOC =
      ...
      + "``TimeBasedPartitioner``. The format set in this configuration converts the Unix "
      + "timestamp "
  ...
  public static final String STORAGE_CLASS_DEFAULT =
  ...
  private static final ConfigDef.Recommender partitionerClassDependentsRecommender =
      new PartitionerClassDependentsRecommender();
  private static final ConfigDef.Recommender schemaCompatibilityRecommender =
    ...
    public boolean visible(String name, Map<String, Object> connectorConfigs) {
      ...
      try {
        ...
        Class<? extends Partitioner>
            partitioner =
            (Class<? extends Partitioner>) Class.forName(partitionerName);
        if (classNameEquals(partitionerName, DefaultPartitioner.class)) {
          ...
          if (classNameEquals(partitionerName, DailyPartitioner.class) || classNameEquals(
              partitionerName,
